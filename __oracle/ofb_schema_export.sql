...
prompt Created by fras200627 on sexta-feira, 11 de julho de 2025
set define off
...
comment on column OFB.CONSENTS_PERSONAL_DATA.expirationdatetimerequested
  is 'Consent expiration date/time. If NULL, consent is valid for an indefinite period.';
comment on column OFB.CONSENTS_PERSONAL_DATA.expirationdatetimeadjusted
  is 'Consent expiration time set to 23:59:59. If NULL, consent is indefinite.';
comment on column OFB.CONSENTS_PERSONAL_DATA.expirationinmonths
  is 'Expiration period in number of months. If NULL, the consent has an indefinite term.';
...
(
  date_default            DATE,
  timestamp_default       TIMESTAMP(6) default SYS_EXTRACT_UTC(SYSTIMESTAMP(3)),
  timestamp_localtimezone TIMESTAMP(6) WITH LOCAL TIME ZONE,
  timestam_timezone       TIMESTAMP(3) WITH TIME ZONE
)
tablespace USERS
  pctfree 10
  initrans 1
...
prompt
prompt Creating view VW_CONSENTS_ACCOUNTS_RESOURCES_AUTHORISED
prompt =======================================================
prompt
CREATE OR REPLACE FORCE VIEW OFB.VW_CONSENTS_ACCOUNTS_RESOURCES_AUTHORISED AS
SELECT
  e.cpfnumber,
  a.consentid,
...
from
  ...
  INNER JOIN OFB.ACCOUNT_PERSONAL_DATA f
...
prompt
prompt Creating view VW_CONSENTS_CUSTOMERS_RESOURCES_AUTHORISED
prompt ========================================================
prompt
CREATE OR REPLACE FORCE VIEW OFB.VW_CONSENTS_CUSTOMERS_RESOURCES_AUTHORISED AS
SELECT
  e.cpfnumber,
  a.consentid,
  b.status             AS consentstatus,
...
SELECT
  a.consentpermissionauthorisedid,
  e.cpfnumber,
  e.civilname          AS customer,
  a.consentid,
  b.status             AS consentstatus,
...
SELECT
  ...
  d.civilname,
  b.status,
  to_char(b.creationdatetime,'YYYY-MM-DD"T"HH24:MI:SS"Z"') AS creationdatetime,
  to_char( b.expirationdatetime,'YYYY-MM-DD"T"HH24:MI:SS"Z"') AS expirationdatetime,
  c.permissioncategorygroup,
  c.permission
from
...
prompt
prompt Creating view VW_CONSENTS_PERSONAL_DATA_RESOURCES_AUTHORISED_ACCOUNTS
prompt =====================================================================
prompt
CREATE OR REPLACE FORCE VIEW OFB.VW_CONSENTS_PERSONAL_DATA_RESOURCES_AUTHORISED_ACCOUNTS AS
SELECT
  a.consentpermissionauthorisedid,
  e.cpfnumber,
  a.consentid,
  ...
  e.civilname          AS customer,
  d.type               AS resourcetype,
  c.resourcepermissionid AS permissionid,
  c.permission,
  c.resourcestatus,
  c.permissioncategorygroup AS resourcesummary,
  c.permissioncategoryid,
  c.permissioneventgroupid,
  c.permissioncategoryorder,
  f.accountid,
...
prompt
prompt Creating view VW_CONSENTS_PERSONAL_DATA_RESOURCES_AUTHORISED_CUSTOMERS
prompt ======================================================================
prompt
CREATE OR REPLACE FORCE VIEW OFB.VW_CONSENTS_PERSONAL_DATA_RESOURCES_AUTHORISED_CUSTOMERS AS
SELECT
  a.consentpermissionauthorisedid,
  e.cpfnumber,
  a.consentid,
...
SELECT
  ...
  a.CONSENTID,
  to_char(d.creationdatetime,'YYYY-MM-DD"T"HH24:MI:SS"Z"') AS CONSENTDATECREATION,
  d.status              AS CONSENTSTATUS,
  to_char(d.expirationdatetime,'YYYY-MM-DD"T"HH24:MI:SS"Z"')  AS CONSENTEXPIRATION,
  d.personalid,
...
/*  SELECT a.resourcestatus INTO resourceStatus 
  ...
  ELSE*/
    IF :NEW.ACCOUNTSTATUS = 'ATIVA' THEN
      UPDATE ofb.consents_personal_data_resources_confirmed a

...
@Component
@Slf4j
public class AccountsGetBalancesByAccountIdProcessor implements Processor {
    ...
    public void process(Exchange exchange) throws Exception {
        ...
        ///
        com.ofb.accounts.client.accounts.model.ResponseAccountBalances responseAccountBalances =
                accountsApi.accountsGetAccountsAccountIdBalances(resourcesAuthorisedList.get(0).getResourceId(),
                        UUID.fromString(exchange.getProperty("x-fapi-interaction-id").toString()));

        ///
        exchange.getMessage().setBody(responseAccountBalances);
    }
